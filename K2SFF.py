...
def K2SFF_VJ14(time,flux, Xc, Yc,CmpStr,starname='',outputpath='',chunksize=75,  PolyDeg=2):
  ...
  Mask = np.where((abs(Xc - np.mean(Xc)) <= 5*np.std(Xc)) & (abs(Yc - np.mean(Yc)) <= 5*np.std(Yc)))
  ...
  ChunkSize = 200 #small chunksize and large chunksize are two different things
  ...
  ThrusterIndices = np.abs(dV)>3.5*Std
  ...
  ChunkSize = 36 #chunk every 1.5 day
