...
def StandardAperture(filepath='',outputpath='',campaign='',plot=False):
    ...
    '''
    Kep1 = FitsFile[1].data['FLUX_BKG']
    Kep2 = FitsFile[1].data['FLUX_ERR']
    Kep3 = FitsFile[1].data['FLUX_BKG_ERR']
    Kep4 = FitsFile[1].data['COSMIC_RAYS']
    MedBKG1_ = np.nanmedian(Kep1, axis=0)
    MedBKG2_ = np.nanmedian(Kep2, axis=0)
    MedBKG3_ = np.nanmedian(Kep3, axis=0)
    MedBKG4_ = np.nanmedian(Kep4, axis=0)
