...
def PlanetFit(t,Flux,Per):
    print "Period:", Per
    #Initializing the parameters
    LMFitparams = Parameters()
    LMFitparams.add('t0', value= 2.65, min=2.6, max=2.8)
    ...
    LMFitparams.add('ecc', value=0., min=0, max=0.1)
    ...
    #For non linear fit
    LMFitparams.add('u1', value= 0.5)
    LMFitparams.add('u2', value= 0.1)
    LMFitparams.add('u3', value= 0.1)
    LMFitparams.add('u4', value= -0.1)
...
def EvenOddTest(EPIC_ID, Time,Flux,period,T0, TWidth):
    '''This function fits the parameters for even and odd period of the planet'''
    ...
    ModelE = PlanetFit(T_EvenFit,F_EvenFit,period)
    ModelO = PlanetFit(T_OddFit,F_OddFit,period)
    ...
    pl.plot(T_EvenFit,ModelE,'r-',lw=3, label="Leastsq Fitting")
    pl.axhline(y=1,color='k',lw=1)
    pl.ylabel('Normalized Flux')
    pl.xlabel('Time(hours)')
    pl.legend(loc='best')
    ...
    pl.subplot(3,1,3)
    pl.plot((T_OddFit-period),F_OddFit,'k.',label="Odd Kepler Data")
    pl.plot((T_OddFit-period),ModelO,'r-',lw=3, label="Leastsq Fitting")
    pl.axhline(y=1,color='k',lw=1)
    pl.ylabel('Normalized Flux')
    pl.xlabel('Time(hours)')
    pl.legend(loc='best')
    pl.suptitle('EPIC:'+EPIC_ID)
    pl.savefig(EPIC_ID+'_Fit.png')

def EvenOddTest_histogram(T_Fold, F_Fold, T_Even, F_Even):
    BinSize = 30
...
def RunStar(EPIC_ID,RUNID):
    ...
    #pl.figure()
    #pl.plot(T_Fold, F_Fold,"k.")
    #pl.show()

    #Determing T0 and TWidth
    T0 = 2.69 #Find the time since conjunction
    TWidth = 0.2
